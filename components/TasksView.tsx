...
import { Plus, Calendar, Send, X, MessageSquare, User, Check, Building2, ChevronLeft } from 'lucide-react';
    ...
    <div className="p-4 space-y-4 pb-12">
      ...
      {isAdding && (
        <div className="fixed inset-0 z-[100] bg-[#020617] flex flex-col p-6 overflow-y-auto custom-scrollbar">
          <button onClick={() => setIsAdding(false)} className="self-start p-2 bg-slate-800 rounded-xl mb-6"><X size={20}/></button>
          <h2 className="text-lg font-black uppercase italic mb-8">Новая задача</h2>
          <div className="space-y-6 flex-1">
            ...
            <input placeholder="Заголовок задачи" value={newTitle} onChange={e => setNewTitle(e.target.value)} className="w-full bg-slate-800/50 p-4 rounded-xl text-sm border border-white/5 font-bold" />
            <textarea placeholder="Описание задачи" value={newDescription} onChange={e => setNewDescription(e.target.value)} className="w-full bg-slate-800/50 p-4 rounded-xl text-sm border border-white/5 min-h-[150px]" />
            <select value={newImportance} onChange={e => setNewImportance(e.target.value as any)} className="w-full bg-slate-800/50 p-4 rounded-xl text-sm border border-white/5">
          ...
          <button onClick={handleCreateTask} className="mt-8 bg-blue-600 w-full p-4 rounded-xl font-black uppercase tracking-widest text-xs">Создать</button>
        </div>
      )}

      {/* Оверлей: Детали и Чат */}
      {editingTask && (
        <div className="fixed inset-0 z-[100] bg-[#020617] flex flex-col pt-safe">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar pb-32">
            <div className="space-y-4">
              <div className="text-[8px] font-black uppercase tracking-[0.3em] text-blue-500">{editingTask.companyName || 'Без компании'}</div>
              ...
              <div className="p-4 bg-slate-800/30 rounded-2xl text-xs text-slate-300 leading-relaxed border border-white/5">
                ...
                {editingTask.description || 'Описание отсутствует'}
              </div>
            </div>
            ...
            <div className="space-y-6 pt-8 border-t border-white/5">
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2">
                <MessageSquare size={14} className="text-blue-500" /> Чат по задаче
              </h3>
              <div className="space-y-4">
                {editingTask.comments?.map(c => (
                  <div key={c.id} className={`flex flex-col ${c.userId === executorId ? 'items-end' : 'items-start'}`}>
                    <div className={`max-w-[85%] p-3 rounded-xl text-xs ${c.userId === executorId ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-200'}`}>
          ...
          <div className="p-4 bg-[#020617] border-t border-white/5 flex gap-2 shrink-0">
            <input 
              placeholder="Написать..." 
              value={commentText} onChange={e => setCommentText(e.target.value)}
              className="flex-1 bg-slate-800/50 p-3 rounded-xl text-xs outline-none" 
            />
            <button onClick={() => handleAddComment(editingTask)} className="bg-blue-600 p-3 rounded-xl"><Send size={18}/></button>
          </div>
        </div>
      )}
    </div>
