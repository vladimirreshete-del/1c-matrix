...
import { Plus, Calendar, Send, X, MessageSquare, ChevronLeft, Trash2 } from 'lucide-react';
...
const TasksView: React.FC<Props> = ({ tasks, team, onAddTask, onUpdateTask, isAdmin, executorId }) => {
  ...
  const handleDeleteTask = (task: Task) => {
    if (confirm('Вы уверены, что хотите удалить эту задачу?')) {
      onUpdateTask({ ...task, status: 'DELETED' as any });
      setEditingTask(null);
    }
  };
      ...
      {isAdding && (
        <div className="fixed inset-0 z-[100] bg-[#020617] flex flex-col p-6 overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-6 flex-1">
            ...
            <textarea placeholder="Описание задачи" value={newDescription} onChange={e => setNewDescription(e.target.value)} className="w-full bg-slate-800/50 p-4 rounded-xl text-sm border border-white/5 min-h-[120px] text-slate-300" />
            
            <div className="grid grid-cols-2 gap-4">
              <select value={newAssignedTo} onChange={e => setNewAssignedTo(e.target.value)} className="w-full bg-slate-800/50 p-4 rounded-xl text-[10px] font-bold text-white border border-white/5">
                {team.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
              </select>
              <select value={newImportance} onChange={e => setNewImportance(e.target.value as any)} className="w-full bg-slate-800/50 p-4 rounded-xl text-[10px] font-bold text-white border border-white/5">
          ...
          <button onClick={handleCreateTask} className="mt-8 bg-blue-600 w-full p-5 rounded-2xl font-black uppercase tracking-widest text-[10px]">Создать</button>
        </div>
      )}

      {editingTask && (
        <div className="fixed inset-0 z-[100] bg-[#020617] flex flex-col pt-safe overflow-hidden">
          <header className="p-4 border-b border-white/5 flex items-center justify-between bg-[#020617] shrink-0">
            <button onClick={() => setEditingTask(null)} className="flex items-center gap-2 text-slate-400 text-[10px] font-black uppercase tracking-widest">
              <ChevronLeft size={18} /> Назад
            </button>
            <span className="text-[10px] font-black uppercase text-slate-600 italic">#{editingTask.number}</span>
            {isAdmin ? (
              <button onClick={() => handleDeleteTask(editingTask)} className="p-2 text-red-500/50 hover:text-red-500 transition-colors">
                <Trash2 size={18} />
              </button>
            ) : <div className="w-8" />}
          </header>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar pb-40">
            <div className="space-y-4">
              <div className="text-[8px] font-black uppercase tracking-[0.3em] text-blue-500">{editingTask.companyName}</div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <select value={editingTask.status} onChange={e => onUpdateTask({...editingTask, status: e.target.value as any})} className="w-full bg-slate-800/50 p-3 rounded-xl text-[10px] font-bold text-white border border-white/5">
          ...
          <div className="p-4 bg-[#020617] border-t border-white/5 flex gap-2 shrink-0 pb-12">
            <input placeholder="Написать..." value={commentText} onChange={e => setCommentText(e.target.value)} className="flex-1 bg-slate-800/50 p-4 rounded-xl text-xs outline-none border border-white/5" />
            <button onClick={() => handleAddComment(editingTask)} className="bg-blue-600 p-4 rounded-xl"><Send size={18}/></button>
          </div>
        </div>
      )}
    </div>
